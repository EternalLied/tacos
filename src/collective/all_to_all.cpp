...
AllToAll::AllToAll(const int npusCount, const int collectivesCount) noexcept : Collective() {
    ...
    // - Chunk j from NPU i needs to be sent to NPU j
    // - Each NPU sends one unique chunk to every other NPU (including itself)
    //
    // Total chunks = npusCount * npusCount * collectivesCount
    // For example, with 4 NPUs and 1 round:
    //   NPU 0: chunks [0,1,2,3] -> destinations [0,1,2,3]
    //   NPU 1: chunks [4,5,6,7] -> destinations [0,1,2,3]
    //   NPU 2: chunks [8,9,10,11] -> destinations [0,1,2,3]
    //   NPU 3: chunks [12,13,14,15] -> destinations [0,1,2,3]

    for (int round = 0; round < collectivesCount; ++round) {
        for (int src = 0; src < npusCount; ++src) {
            for (int dest = 0; dest < npusCount; ++dest) {
                // Each chunk goes to exactly one destination
